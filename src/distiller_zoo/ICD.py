...
class ICD(KDWrapper):
    """
    Distillation with multi-head attention. Mimic attention and features. Instance Conditional Distillation, NeurIPS 2021
    """
    ...
    def __init__(self, cfg, student, teacher) -> None:
        ...
        self.weight_value = cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.WEIGHT_VALUE
        self.temp_value = cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.TEMP_VALUE
        if self.temp_value < 0:
            self.temp_value = nn.Parameter(torch.ones([1]).mean())

        self.distill_norm_type = cfg.MODEL.DISTILLER.KD.DISTILL_NORM

        self.distill_negative = cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.DISTILL_NEGATIVE
        self.use_pos_embds = cfg.MODEL.DISTILLER.KD.USE_POS_EMBEDDING

        self.predictor = MLP(hidden_dim, hidden_dim, 1, 3)

        if self.distill_norm_type == 'ln':
            self.distill_norm_ = nn.LayerNorm(
                [hidden_dim // cfg.MODEL.DISTILLER.KD.ATT_HEADS], elementwise_affine=False)
        ...
        self.loss_form = cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.LOSS_FORM
    ...
    def forward(self, features_dict, features_dict_tea):
        if isinstance(self.temp_value, nn.Parameter):
            ...
            if self.cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.TEMP_DECAY:
                decay_to = self.cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.TEMP_DECAY_TO
                ratio = features_dict['iteration'] / self.cfg.SOLVER.MAX_ITER
                self.temp_value = ratio * decay_to + \
                    (1 - ratio) * self.cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.TEMP_VALUE
