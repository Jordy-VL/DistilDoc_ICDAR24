...
import math
from .ICD_MHA_modules import MultiheadAttention
import json
...
def build_instance_encoder(cfg):
    ...
    """
    meta_arch = cfg.MODEL.DISTILLER.KD.TASK_NAME
...
def build_decoder_module(cfg):
    ...
    Note that it does not load any weights from ``cfg``.
    """
    meta_arch = cfg.MODEL.DISTILLER.KD.DECODER
...
class OfficialDecoderLayer(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        channels = cfg.MODEL.DISTILLER.KD.HIDDEN_DIM
        heads = cfg.MODEL.DISTILLER.KD.ATT_HEADS
        layers = cfg.MODEL.DISTILLER.KD.ATT_LAYERS

        self.matt = MultiheadAttention(
            channels, heads, dropout=cfg.MODEL.DISTILLER.KD.DROPOUT)

        if cfg.MODEL.DISTILLER.KD.PROJECT_POS:
            self.pos_projector = nn.Linear(channels, channels)
        else:
            self.pos_projector = nn.Sequential()

        self.use_pos = cfg.MODEL.DISTILLER.KD.USE_POS_EMBEDDING
        ...
        self.pos_on_v = cfg.MODEL.DISTILLER.KD.DECODER_POSEMB_ON_V
...
class InstanceEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.sample_rule = cfg.MODEL.DISTILLER.KD.SAMPLE_RULE
        self.neg_ratio = cfg.MODEL.DISTILLER.KD.NUM_NEG_POS_RATIO
        self.total_boxes = cfg.MODEL.DISTILLER.KD.NUM_LABELS
        self.max_boxes = cfg.MODEL.DISTILLER.KD.MAX_LABELS

        self.num_pos_feats = 128
        self.scale = 2 * math.pi
        self.temperature = 10000

        self.num_classes = cfg.MODEL.DISTILLER.KD.NUM_CLASSES

        if self.num_classes != 80 or cfg.MODEL.DISTILLER.KD.UNIFORM_SAMPLE_CLASS:
        ...
        if self.num_classes != 80 or cfg.MODEL.DISTILLER.KD.UNIFORM_SAMPLE_BOX:
            self.coco_pos_prior = np.ones(
                [self.num_classes, 2, 4], dtype=np.float32)
            self.coco_pos_prior[:, :, :] = 0.2
            # H, W: mean 0.5, std 0.2

        self.add_scale_indicator = cfg.MODEL.DISTILLER.KD.ADD_SCALE_INDICATOR
        ...
        self.conf_weight = cfg.MODEL.DISTILLER.KD.CONFIDENCE_LOSS_WEIGHT
        self.sample_range = cfg.MODEL.DISTILLER.KD.INS_ATT_MIMIC.SAMPLE_RANGE
