...
from odmodeling.build import build_distill_configs
...
# import to register
import odmodeling.distiller
import odmodeling.model
import odmodeling.teacher
import arch_utils
...
DATAROOT = '/data2/users/abanerjee/DocLayNet_core'
...
def do_train(cfg, model, teacher, resume=False):
    ...
    # cfg_ = cfg.MODEL.DISTILLER
    # cfg_ = odmodeling.teacher.add_defaults_config(cfg_)
    teacher_opt = my_build_optimizer(cfg.MODEL.DISTILLER, teacher)
    teacher_sche = build_lr_scheduler(cfg.MODEL.DISTILLER, teacher_opt)         # TODO: Check error with scheduler if any!
    ...
    if start_iter == 0:
        ...
        if cfg.MODEL.DISTILLER.PRELOAD_TEACHER != '':
            ...
            assert cfg.MODEL.DISTILLER.PRELOAD_TYPE in ['all', 'teacher_only', 'none', 'vanilla']
            if cfg.MODEL.DISTILLER.PRELOAD_TYPE == 'all':
                ...
                if comm.is_main_process():
                    ...
                    logger.warning(
                        'Incompatible: <%s> \n Unexpected: <%s>' % (mskeys, unexp_keys))
            elif cfg.MODEL.DISTILLER.PRELOAD_TYPE == 'vanilla':
                # DetectionCheckpointer(tea_module.teacher., save_dir='teacher_outputs').resume_or_load(cfg.MODEL.DISTILLER.PRELOAD_TEACHER, resume=args.resume)
                pass
            elif cfg.MODEL.DISTILLER.PRELOAD_TYPE == 'none':
                print('Teacher already loaded in ModelTeacher!')
    ...
    with EventStorage(max(0, start_iter - pretrain_iter)) as storage:
        for data, _iteration in zip(data_loader, range(start_iter, max_iter + pretrain_iter)):
            if _iteration < pretrain_iter:
                ...
                # losses = torch.sum(torch.stack(list(loss_dict.values())))         # alternate computation for MacOS
            ...
            images = [x['image'].to(torch.device(cfg.MODEL.DEVICE)) for x in data]
            ...
            if cfg.MODEL.DISTILLER.DISTILL_FLAG == 'distill_only':
                loss_dict = {} # resetting supervised losses to zero
                loss_dict.update(loss_tea_dict)
            elif cfg.MODEL.DISTILLER.DISTILL_FLAG == 'distill_and_supervision':
                loss_dict.update(loss_tea_dict)
            elif cfg.MODEL.DISTILLER.DISTILL_FLAG == 'supervision_only':
                del loss_tea_dict
            else:
            ...
            if fix_head:
                for p in module.proposal_generator.parameters():
                    p.grad = None
                
                for p in module.roi_heads.parameters():
                    p.grad = None
...
def main(args):
    # Registering DocLayNet
    register_coco_instances(
        "doclaynet_train",
        {},
        f"{DATAROOT}/COCO/train.json",
        f"{DATAROOT}/PNG"
    )
    ...
    register_coco_instances(
        "doclaynet_test",
        {},
        f"{DATAROOT}/COCO/test.json",
        f"{DATAROOT}/PNG"
    )

    MetadataCatalog.get("doclaynet_train").thing_classes = ['Caption', 'Footnote', 'Formula', 'List-item', 'Page-footer', 'Page-header', 'Picture', 'Section-header', 'Table', 'Text', 'Title']
    ...
    teacher = odmodeling.model.Distillator(cfg, model)
